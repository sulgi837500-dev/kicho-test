...
for key in ['results', 'current_q', 'score', 'step', 'start_time', 'end_test']:
    if key not in st.session_state: 
        st.session_state[key] = [] if key == 'results' else (0 if key in ['current_q', 'score'] else ("setup" if key == 'step' else (None if key == 'start_time' else False)))

# 2. 상단 제목 (공식 명칭) [cite: 1-7, 754-762]
st.markdown("""
...
with st.sidebar:
    ...
    if st.button("🔄 검사 초기화"):
        ...
        st.session_state.results = []; st.session_state.start_time = None; st.session_state.end_test = False; st.rerun()

# 4. 전 문항 데이터베이스 (PDF 전수 반영)
# [수학 문항: cite 82-123, 128-155, 170-200, 205-234, 330-359, 363-393]
...
math_subtraction_9 = ["3-1", "2-1", "3-2", "5-2", "5-3", "3-3", "4-2", "5-4", "4-3", "6-1", "9-5", "8-4", "7-2", "8-5", "9-7", "8-7", "9-4", "9-2", "8-3", "7-0", "7-4", "7-5", "9-3", "7-6", "8-6", "8-1", "6-4", "8-2", "6-3", "9-6"]
math_carry_up = ["9+1", "9+3", "7+3", "9+2", "8+2", "6+6", "3+8", "6+4", "2+9", "7+9", "6+8", "4+7", "6+5", "8+6", "9+4", "9+5", "4+8", "5+7", "8+9", "7+6", "4+9", "7+5", "5+9", "7+7", "9+7", "7+8", "9+9", "8+8", "5+8", "6+9"]
math_borrow_down = ["10-1", "11-3", "10-5", "10-8", "12-3", "11-9", "12-4", "11-8", "11-2", "12-9", "13-6", "11-6", "13-5", "11-5", "18-9", "12-8", "15-8", "14-9", "14-7", "13-9", "13-7", "12-6", "11-7", "16-8", "12-7", "16-9", "14-8", "11-4", "15-6", "17-8"]
...
# [국어 문항 전수 로드: cite 767, 837, 863, 942, 1155, 1268]
kor_decoding = ["ㅏ", "ㅓ", "ㅗ", "ㅜ", "ㅡ", "ㅣ", "ㅐ", "ㅔ", "ㅑ", "ㅕ", "ㄱ", "ㄴ", "ㄷ", "ㄹ", "ㅁ", "ㅂ", "ㅅ", "ㅇ", "ㅈ", "ㅊ", "ㅋ", "ㅌ", "ㅍ", "ㅎ", "ㄲ", "ㄸ", "ㅃ", "ㅆ", "ㅉ", "가", "나", "다", "라", "마", "바", "사", "아", "자", "차", "카", "타", "파", "하", "어머니", "나무", "소", "우유", "아이", "나버", "계난", "포모", "연팔", "책성"]
...
kor_passage_elem = ["땀이 나는 이유(제목)", "여러분,", "땀을", "흘려", "본", "경험이", "있지요?", "우리는", "여러", "가지", "상황에서", "땀을", "흘립니다.", "땀은", "왜", "나는", "것일까요?"]
kor_passage_sec = ["반려동물 관련 직업 세계(제목)", "사람과", "더불어", "살아가기", "위한", "목적으로", "기르는", "동물을", "반려동물이라고", "합니다."]
kor_comp_sec = [{"text": "사람과 인공지능의 대결은 사람들의 많은 관심을", "opts": ["받아왔습니다", "선물", "상쾌한"], "ans": "받아왔습니다"}, {"text": "과연 인공지능이 사람을 이길 수 있을까요?", "opts": ["개인적인", "대표적인", "긍정적인"], "ans": "대표적인"}]

# 문항 할당 로직
if "수학" in category:
    q_map = {"① 9 이하 덧셈": math_addition_9, "② 9 이하 뺄셈": math_subtraction_9, "③ 받아올림 덧셈": math_carry_up, "④ 받아내림 뺄셈": math_borrow_down, "⑦ 곱셈구구(2~5단)": math_mul_25, "⑧ 곱셈구구(6~9단)": math_mul_69}
    questions = q_map.get(sub_category, [])
else:
    if "해득" in sub_category: questions = kor_decoding
    elif "무의미" in sub_category: questions = kor_nonsense_all[:40] if "초등 1학년" in grade or "초등 2학년" in grade else kor_nonsense_all
...
# 5. 검사 실행 화면 및 초시계 [cite: 74-77, 831-832]
if not name:
    st.warning("👈 왼쪽 사이드바에 학생 정보를 입력해 주세요.")
elif st.session_state.step == "setup":
    st.subheader(f"📢 {sub_category} 학생 안내")
    st.info("지우개 사용 금지. 선생님 신호에 맞춰 최대한 정확하고 빠르게 수행하세요.")
    ...
    col_t1.metric("⏱️ 남은 시간", f"{rem}초")
    st.progress(rem / limit_sec)
    
    if rem <= 0:
        st.error("⏰ 시간이 종료되었습니다!"); st.session_state.end_test = True
    
    if st.session_state.current_q < len(questions) and not st.session_state.end_test:
        ...
        if isinstance(q, dict): # 중등 읽기 이해
            ...
            for i, opt in enumerate(q['opts']):
                if cols[i].button(opt, key=f"c_{i}_{st.session_state.current_q}", use_container_width=True):
                    ...
                    st.session_state.results.append({"번호": st.session_state.current_q+1, "문항": q['text'], "선택": opt, "정오": res, "속도": "-", "비고": "-"})
    ...
    # 도달 기준 자동 판정 (PDF 근거) [cite: 49, 808-811, 1065]
    ...
    st.metric(f"{sub_category} 최종 점수", f"{st.session_state.score}점", f"판정 결과: {status}")
    
    df_detail = pd.DataFrame(st.session_state.results)
    summary_data = pd.DataFrame([{"항목": sub_category, "학년": grade, "시기": period, "총점": st.session_state.score, "판정": status}])

    # 엑셀 시트 이름별 저장
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        summary_data.to_excel(writer, sheet_name='진단보고서_요약', index=False)
        df_detail.to_excel(writer, sheet_name=sub_category[:31], index=False)
    
    st.table(df_detail)
    st.download_button("📥 엑셀 보고서 다운로드", output.getvalue(), f"인천교육청_{name}_{sub_category}.xlsx")
