...
# 2. 메인 안내 및 지필평가 고지 [cite: 754-762]
st.markdown("""
...
# ==========================================
# [기초 수학 8단계: 215문항 전수 반영] [cite: 83-393]
MATH_STEPS = {
    "① 9 이하 덧셈": {"qs": ["2+1", "1+4", "1+1", "3+2", "3+1", "5+0", "1+3", "4+1", "0+3", "6+1", "1+5", "6+2", "8+1", "3+3", "4+4", "2+4", "4+3", "4+5", "3+5", "2+7", "2+6", "5+4", "1+6", "3+4", "7+1", "6+3", "5+3", "5+2", "4+2", "7+2"], "time": 60, "pass": 18, "page": "수학 PDF 23쪽"},
    "② 9 이하 뺄셈": {"qs": ["3-1", "2-1", "3-2", "5-2", "5-3", "3-3", "4-2", "5-4", "4-3", "6-1", "9-5", "8-4", "7-2", "8-5", "9-7", "8-7", "9-4", "9-2", "8-3", "7-0", "7-4", "7-5", "9-3", "7-6", "8-6", "8-1", "6-4", "8-2", "6-3", "9-6"], "time": 60, "pass": 13, "page": "수학 PDF 25쪽"},
    "③ 받아올림 덧셈": {"qs": ["9+1", "9+3", "7+3", "9+2", "8+2", "6+6", "3+8", "6+4", "2+9", "7+9", "6+8", "4+7", "6+5", "8+6", "9+4", "9+5", "4+8", "5+7", "8+9", "7+6", "4+9", "7+5", "5+9", "7+7", "9+7", "7+8", "9+9", "8+8", "5+8", "6+9"], "time": 60, "pass": 9, "page": "수학 PDF 27쪽"},
    "④ 받아내림 뺄셈": {"qs": ["10-1", "11-3", "10-5", "10-8", "12-3", "11-9", "12-4", "11-8", "11-2", "12-9", "13-6", "11-6", "13-5", "11-5", "18-9", "12-8", "15-8", "14-9", "14-7", "13-9", "13-7", "12-6", "11-7", "16-8", "12-7", "16-9", "14-8", "11-4", "15-6", "17-8"], "time": 60, "pass": 6, "page": "수학 PDF 29쪽"},
    "⑤ 몇십 몇+몇십 몇": {"qs": ["18+42", "65+88", "64+85", "31+99", "54+53", "63+70", "12+97", "41+89", "28+39", "42+76", "73+56", "18+27", "85+57", "44+49", "79+74", "93+99", "73+17", "58+99", "24+80", "68+16"], "time": 120, "pass": 7, "page": "수학 PDF 31~32쪽"},
    "⑥ 몇십 몇-몇십 몇": {"qs": ["47-29", "72-15", "56-27", "92-87", "60-49", "71-55", "76-68", "50-34", "52-48", "77-58", "91-53", "37-19", "45-28", "83-67", "51-39"], "time": 120, "pass": 4, "page": "수학 PDF 34쪽"},
    "⑦ 곱셈구구(1)": {"qs": [f"{a}x{b}" for a, b in [(3,1), (5,2), (2,2), (5,1), (4,2), (5,4), (2,3), (5,3), (2,4), (4,4), (3,3), (3,5), (4,5), (5,5), (2,6), (5,6), (2,7), (5,7), (2,8), (5,8), (2,9), (4,3), (3,7), (3,9), (4,8), (3,6), (4,9), (3,8), (4,6), (3,4)]], "time": 60, "pass": 16, "page": "수학 PDF 36쪽"},
    "⑧ 곱셈구구(2)": {"qs": [f"{a}x{b}" for a, b in [(6,1), (9,2), (8,2), (8,5), (9,1), (9,5), (6,5), (7,2), (7,5), (6,3), (9,4), (6,4), (7,3), (8,3), (9,3), (6,6), (8,4), (7,7), (7,4), (8,8), (9,7), (6,7), (8,9), (7,6), (6,9), (7,9), (8,6), (9,6), (6,8), (9,8)]], "time": 60, "pass": 11, "page": "수학 PDF 18쪽"}
}

# [기초 국어 10단계 92문항 전수 반영] [cite: 767, 794-803]
KOR_STEPS = {
    "1단계: 모음": {"qs": ["ㅏ", "ㅓ", "ㅗ", "ㅜ", "ㅡ", "ㅣ", "ㅐ", "ㅔ", "ㅑ", "ㅕ"], "page": "국어 PDF 한글해득 학생용"},
    "2단계: 자음": {"qs": ["ㄱ", "ㄴ", "ㄷ", "ㄹ", "ㅁ", "ㅂ", "ㅅ", "ㅇ", "ㅈ", "ㅊ", "ㅋ", "ㅌ", "ㅍ", "ㅎ", "ㄲ", "ㄸ", "ㅃ", "ㅆ", "ㅉ"], "page": "국어 PDF 한글해득 학생용"},
...
# 5. 메인 메뉴 로직
if not name:
    st.warning("👈 사이드바에 학생 이름을 입력하여 시작하세요.")
...
# --- [가이드 화면 (KeyError 핵심 해결 구역)] ---
    ...
    if target in MATH_STEPS:
        ...
        next_step = "test"
    elif target in NONSENSE_DB:
        st.warning(f"⏱️ 제한 시간: 40초 | 최대한 정확하고 빠르게 | 📄 지필평가: {NONSENSE_DB[target]['page']}")
        next_step = "test"
    elif target in PASSAGES:
        st.warning(f"⏱️ 제한 시간: 1분 | 제목부터 읽기 시작 | 📄 지필평가: {PASSAGES[target]['page']}")
        next_step = "passage_test"
    elif target == "중등 읽기 이해":
...
# --- [통합 테스트 실행 엔진 (수학, 한글해득, 무의미단어)] ---
    ...
    elif target in KOR_STEPS: qs = KOR_STEPS[target]['qs']; limit = 0; is_timer = False
    elif target in NONSENSE_DB: qs = NONSENSE_DB[target]['qs']; limit = 40; is_timer = True
    ...
    if st.session_state.current_q >= len(qs): st.session_state.step = "res"; st.rerun()
    
    q = qs[st.session_state.current_q]
    st.markdown(f"<h1 style='text-align: center; font-size: 150px;'>{q}</h1>", unsafe_allow_html=True)
    
    with st.expander("📝 질적 분석 (교사용 기록)", expanded=True):
        spd = st.radio("반응 속도", ["즉각적", "느림"], horizontal=True, key=f"s_{st.session_state.current_q}")
        act = st.text_input("아동 반응", key=f"a_{st.session_state.current_q}")
...
# --- [중등 읽기 이해 전용 엔진] ---
    ...
    target = st.session_state.sub_target
    qs = COMPREHENSION_DB[target]['qs']
    if st.session_state.current_q >= len(qs): st.session_state.step = "res"; st.rerun()
    
    item = qs[st.session_state.current_q]
...
# --- [결과 판정 및 경로 제어판] ---
    ...
    if target in MATH_STEPS:
        ...
        else: st.error(f"❌ **미도달** ({score}점 / 기준 {pass_mark}점). 이 단계가 기초선입니다.")
    elif target in KOR_STEPS:
        score = st.session_state.total_score
        if score == len(KOR_STEPS[target]['qs']): st.success("✅ **도달** (All-Pass). 다음 단계 진단 가능.")
        else: st.error(f"❌ **미도달** ({score}점). 보충 지도가 필요합니다.")
    elif "설명문" in target:
        wcpm = st.session_state.total_read - st.session_state.error_word
        st.info(f"결과: 낭독 어절 {st.session_state.total_read}개 / 오류 {st.session_state.error_word}개 ➔ **{wcpm} WCPM**")
    else:
        st.info(f"검사 완료! ({st.session_state.total_score} 문항 응답)")
    ...
    if target in MATH_STEPS:
        ...
        if curr_idx + 1 < len(steps) and c1.button(f"➡️ 다음 단계 ({steps[curr_idx+1]})", use_container_width=True):
            st.session_state.sub_target = steps[curr_idx+1]; st.session_state.step = "guide"; st.rerun()
        if c2.button("🔄 다시 진단", use_container_width=True): st.session_state.step = "guide"; st.rerun()
        if c3.button("🏠 메인으로", use_container_width=True): st.session_state.step = "setup"; st.rerun()
    else:
        if c1.button("⬅️ 국어 메뉴로", use_container_width=True): st.session_state.step = "kor_menu"; st.rerun()
        if c2.button("🔄 다시 진단", use_container_width=True): st.session_state.step = "guide"; st.rerun()
        if c3.button("🏠 메인으로", use_container_width=True): st.session_state.step = "setup"; st.rerun()
