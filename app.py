...
math_2digit_add = ["18+42", "65+88", "64+85", "31+99", "54+53", "63+70", "12+97", "41+89", "28+39", "42+76", "73+56", "18+27", "85+57", "44+49", "79+74", "93+99", "73+17", "58+99", "24+80", "4+80"]
...
kor_hangeul = {
    ...
    "4단계: 받침 없는 단어(의미/무의미)": ["아파", "거미", "효자", "라디오", "배나무", "프소", "가야", "유하", "묘시", "녀타"],
    "5단계: 복잡한 모음": ["ㅘ", "ㅝ", "ㅚ", "ㅟ", "ㅢ", "ㅒ", "ㅖ", "ㅙ", "ㅞ", "ㅛ"],
    "6~9단계: 받침 단어": ["안", "암", "알", "압", "앗", "악", "앙", "밖", "있", "읽", "앉", "삶", "넓", "값", "선생님", "학교", "친구", "공부", "사랑", "달팽미", "발차국", "준비물", "운동정", "우리너라"],
...
if not name:
    ...
    st.markdown("### 📌 권장 진단 순서 및 지침")
    st.info("한글 해득수준(10단계) 통과 후 유창성 검사를 진행하는 것을 권장합니다.")
    
    col1, col2, col3 = st.columns(3)
    if col1.button("📖 한글 해득 수준", use_container_width=True):
        st.session_state.step = "h_guide"; st.session_state.path_step = "1단계: 모음"; st.rerun()
    if col2.button("⏱️ 유창성/이해 검사", use_container_width=True):
        st.session_state.step = "nonsense_guide"; st.rerun()
    if col3.button("🔢 기초 수학 연산", use_container_width=True):
        st.session_state.step = "math_guide"; st.rerun()

# [기초 국어 로직]
# 1. 한글 해득 All-Pass (타이머 없음) [cite: 791, 794-803]
    ...
    if st.session_state.step == "h_guide":
        ...
        actual = st.text_input("아동 발화 기록 (오독 시 기재)", key=f"a_{st.session_state.current_q}")
        c1, c2 = st.columns(2)
        if c1.button("⭕ 정답", use_container_width=True, type="primary"):
            st.session_state.results.append({"번호": st.session_state.current_q+1, "보기": q, "목표": q, "반응": actual if actual else q, "속도": speed, "점수": 1})
        ...
        if c2.button("❌ 오답", use_container_width=True):
            st.session_state.results.append({"번호": st.session_state.current_q+1, "보기": q, "목표": q, "반응": actual, "속도": speed, "점수": 0})
        ...
        if is_pass:
            ...
            if idx + 1 < len(lv_list) and st.button("➡️ 다음 레벨"):
                st.session_state.update({"path_step": lv_list[idx+1], "current_q": 0, "level_score": 0, "step": "h_test"}); st.rerun()
            else: st.balloons(); st.button("🏁 전체 결과 저장", on_click=lambda: st.session_state.update({"step": "final"}))
        else:
            st.error("❌ 미도달 (기초선 확인)"); st.markdown(f"**💡 고려사항:** {level} 단계 결손 발생. 기초선 보충 지도가 필요합니다. [cite: 794-803]")
            if st.button("📥 현재까지 결과 저장"): st.session_state.step = "final"; st.rerun()

# 2. 유창성 (설명문 카운터 및 실시간 타이머) [cite: 884, 901-903]
elif "nonsense" in st.session_state.step:
    passage = kor_passage_sec if "중등" in grade or "고등" in grade else kor_passage_elem
    if "guide" in st.session_state.step:
        st.subheader("⏱️ 설명문 유창성 진단")
        st.markdown("> **검사자:** 너무 빠르지 않게 정확하게 읽으세요. 1분 타이머 작동합니다. [cite: 887-888]")
        if st.button("시작"): st.session_state.start_time = time.time(); st.session_state.step = "nonsense_test"; st.rerun()
        ...
        if c3.button("🏁 읽기 종료") or rem <= 0:
            st.session_state.elapsed_time = int(elapsed); st.session_state.step = "final"; st.rerun()
...
# 3. 최종 저장 및 질적 분석 엑셀
    ...
    # 보고서 요약 통계 [cite: 909-912]
    correct_wcpm = st.session_state.total_read_count - st.session_state.error_count
    if st.session_state.total_read_count > 0:
        st.success(f"유창성 결과: {correct_wcpm} WCPM (정확도 {correct_wcpm/st.session_state.total_read_count*100:.1f}%)")
    
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df.to_excel(writer, sheet_name='질적분석_기록지', index=False)
    st.table(df)
    st.download_button("📥 질적 분석용 엑셀 다운로드", output.getvalue(), f"{name}_진단보고서.xlsx")
